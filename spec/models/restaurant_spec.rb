...
describe Restaurant do
  describe "#available?" do
    ...
    it "should return false if there's exactly the capacity of the restaurant" do
      expect(restaurant.available?(100, start_time)).to be(true)
    end

    context "with one existing reservation" do
      let!(:reservation) { Reservation.create(:restaurant => restaurant, :reservation_time => start_time, :party_size => 10)}
